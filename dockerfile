FROM hub.agcs/dockerhub-proxy/library/node:16 AS build
...
USER node
RUN npm set registry https://<REGISTRY>/
...
FROM hub.agcs/dockerhub-proxy/library/node:16-alpine AS release
